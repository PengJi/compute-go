...
// &str 有两个值：str 的地址和长度, &str 的大小在编译时就可以知道，长度就是 2*usize
// 动态大小类型的黄金规则：必须将动态大小类型的值置于某种指针之后，如：Box<str>\Rc<str>

// 另一个动态大小类型是 trait。每一个 trait 都是一个可以通过 trait 名称来引用的动态大小类型。
// 为了将 trait 用于 trait 对象，必须将他们放入指针之后，
// 比如 &Trait 或 Box<Trait>（Rc<Trait> 也可以）。

//（2） Sized trait
// 为了处理 DST，Rust 用 Sized trait 来决定一个类型的大小是否在编译时可知。
...
fn generic<T: ?Sized>(t: &T) {  // T 可能是 Sized，也可能不是 Sized 的
    // --snip--
}