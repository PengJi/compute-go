...
Python通过`copy`模块中的`copy`和`deepcopy`函数来实现浅拷贝和深拷贝操作，其中`deepcopy`可以通过`memo`字典来保存已经拷贝过的对象，从而避免刚才所说的自引用递归问题；此外，可以通过`copyreg`模块的`pickle`函数来定制指定类型对象的拷贝行为。
...
浅拷贝创建一个新对象，但新对象仅仅是对原对象中元素的引用。
这意味着，对于包含可变对象（如列表或字典）的原对象，浅拷贝后的新对象中的元素仍然指向原对象中的那些可变对象。如果你修改这些可变对象，新对象和原对象都会受到影响。

#### 创建浅拷贝的方法：

1. 使用 `copy` 模块的 `copy()` 方法。
2. 使用切片（对于列表）。
3. 使用工厂函数（如 `list()`、`dict()`）。
...
Shallow Copy 1: [1, 2, ['changed', 4]]
Shallow Copy 2: [1, 2, ['changed', 4]]
Shallow Copy 3: [1, 2, ['changed', 4]]
```

如你所见，修改 `original_list` 中的嵌套列表的元素后，所有浅拷贝的列表也受到了影响。

### 深拷贝（Deep Copy）

深拷贝创建一个新对象，并递归地复制原对象中的所有对象，确保新对象与原对象完全独立。
修改深拷贝后的新对象不会影响原对象，反之亦然。
...
如你所见，修改 `original_list` 中的嵌套列表的元素后，深拷贝的列表没有受到影响。

### 总结

- **浅拷贝**：只复制了对象的引用，新对象与原对象共享其中的可变元素。对这些可变元素的修改会影响到所有引用它们的对象。
- **深拷贝**：递归地复制所有元素，创建完全独立的新对象。修改新对象不会影响原对象，反之亦然。

选择浅拷贝还是深拷贝取决于你是否需要独立地修改拷贝后的对象。如果你需要独立的副本，应使用深拷贝。如果你只需要复制顶层对象的结构，并且知道嵌套的可变对象不会被修改，可以使用浅拷贝。
...
Python 中的迭代器和生成器是用于简化遍历数据集合和创建数据流的强大工具。
迭代器是实现了迭代器协议的对象。跟其他编程语言不同，Python中没有用于定义协议或表示约定的关键字，像`interface`、`protocol`这些单词并不在Python语言的关键字列表中。
Python语言通过魔法方法来表示约定，也就是我们所说的协议，而`__next__`和`__iter__`这两个魔法方法就代表了迭代器协议。
...
#### 概念
- **迭代器**是一个实现了迭代协议的对象，该协议包括 `__iter__()` 方法和 `__next__()` 方法。
- 迭代器是一次性对象，只能向前推进，不能回退。

#### 创建迭代器
1. **通过实现 `__iter__()` 和 `__next__()` 方法创建自定义迭代器**。
2. **通过 `iter()` 函数将可迭代对象（如列表、元组等）转换为迭代器**。
...
#### 概念
- **生成器**是一个特殊类型的迭代器，通过使用 `yield` 关键字的函数创建。
- 生成器函数在每次调用 `yield` 时暂停其执行状态，并返回一个值。再次调用时从暂停的位置继续执行。
...
for value in gen_exp:
    print(value)  # 输出: 4 9 16
```

### 迭代器与生成器的比较

- **迭代器**：
  - 通常需要实现 `__iter__()` 和 `__next__()` 方法。
  - 可以从任何类创建。
  - 需要显式维护状态。

- **生成器**：
  - 使用 `yield` 关键字创建，自动实现了迭代器协议。
  - 更加简洁易读。
  - 自动维护状态，每次暂停和恢复时保留上下文。

### 使用场景

- **迭代器**适用于需要自定义复杂迭代行为的情况。
- **生成器**适用于需要生成大量数据，但又不希望一次性将数据全部加载到内存中的情况，如读取大文件、生成无限序列等。

### 内存效率

生成器由于采用惰性求值（lazy evaluation），在处理大型数据集或流数据时，比列表、元组等更节省内存。
...
首先需要注意的是`multiply`函数用生成式语法返回了一个列表，列表中保存了4个Lambda函数，这4个Lambda函数会返回传入的参数乘以`i`的结果。需要注意的是这里有闭包（closure）现象，`multiply`函数中的局部变量`i`的生命周期被延展了，由于`i`最终的值是`3`，所以通过`m(100)`调列表中的Lambda函数时会返回`300`，而且4个调用都是如此。
...
> **点评**：C++、Java、C#等诸多编程语言都支持函数重载，所谓函数重载指的是在同一个作用域中有多个同名函数，它们拥有不同的参数列表（参数个数不同或参数类型不同或二者皆不同），可以相互区分。
重载也是一种多态性，因为通常是在编译时通过**参数的个数和类型**来确定到底调用哪个重载函数，所以也被称为编译时多态性或者叫前绑定。
...
> **点评**：**生成式（推导式）属于Python的特色语法之一，几乎是面试必考内容**。
> Python中通过生成式字面量语法，可以创建出列表、集合、字典。
...
> 接下来，通过`Child1.x = 2`给类`Child1`重新绑定了属性`x`并赋值为`2`，所以`Child1.x`会输出`2`，而`Parent`和`Child2`并不受影响。
> 执行`Parent.x = 3`会重新给`Parent`类的`x`属性赋值为`3`，由于`Child2`的`x`属性继承自`Parent`，所以`Child2.x`的值也是`3`；
> 而之前我们为`Child1`重新绑定了`x`属性，那么它的`x`属性值不会受到`Parent.x = 3`的影响，还是之前的值`2`。
...
`__new__`方法的返回值是创建好的Python对象（的引用），而`__init__`方法的第一个参数就是这个对象（的引用），所以在`__init__`中可以完成对对象的初始化操作。
`__new__`是类方法，它的第一个参数是类，`__init__`是对象方法，它的第一个参数是对象。
...
在 Python 编程中，鸭子类型（Duck Typing）是一种动态类型的方式，它强调对象的行为（方法和属性）而不是对象本身的类型。
简单来说，“如果它走起来像鸭子，叫起来像鸭子，那么它就是鸭子”。
这意味着，只要一个对象实现了所需的方法和属性，它就可以被视为满足某种类型，而不必显式地继承某个类或实现某个接口。
示例：
```python
class Duck:
    def quack(self):
...
Python中有四种作用域，分别是局部作用域（**L**ocal）、嵌套作用域（**E**mbedded）、全局作用域（**G**lobal）、内置作用域（**B**uilt-in），
搜索一个标识符时，会按照**LEGB**的顺序进行搜索，如果所有的作用域中都没有找到这个标识符，就会引发`NameError`异常。

## 题目28：说一下你对闭包的理解。

闭包是指在函数内部定义的函数，可以引用外部函数的变量，并且即使外部函数已经返回，内部函数仍然可以访问这些变量。
闭包是一种强大的编程工具，它可以用于创建函数工厂、实现数据封装等。

### 闭包的构成要素

1. **嵌套函数**：在一个函数内部定义的函数。
2. **自由变量**：外部函数中的变量，被内部函数引用。
3. **外部函数返回内部函数**：外部函数返回值是内部定义的函数。

### 闭包的原理

当外部函数返回内部函数时，内部函数会记住外部函数中的自由变量，这些变量的生命周期会延长到内部函数被调用结束为止。
...
- `inner_function` 是在 `outer_function` 内部定义的嵌套函数，接受一个参数 `y`。
- `inner_function` 引用了 `outer_function` 中的自由变量 `x`。
- `outer_function` 返回 `inner_function`，形成闭包。

即使 `outer_function` 返回后，其作用域内的变量 `x` 仍然被 `inner_function` 记住，并在后续调用时使用。

### 闭包的应用

#### 1. 函数工厂

闭包可以用来创建函数工厂，根据传入参数动态创建不同功能的函数。
...
1. **自由变量的生命周期**：自由变量的生命周期与闭包函数的生命周期一致，可能导致意外的内存占用。
2. **使用 `nonlocal` 关键字**：在 Python 3.x 中，如果要在闭包内部修改外部函数的变量，需要使用 `nonlocal` 关键字。
...
1. **数据隐藏**：可以封装变量，防止外部访问。
2. **函数工厂**：可以动态创建带有特定状态的函数。
3. **函数式编程**：简化了函数式编程中的某些模式。

#### 缺点

1. **内存占用**：由于闭包会持有对外部函数变量的引用，可能导致意外的内存占用。
2. **调试困难**：闭包涉及多层次的函数调用，调试时可能不直观。

### 总结

闭包是 Python 中一个重要的概念，它通过嵌套函数和自由变量实现数据封装和函数工厂等高级功能。尽管有一定的复杂性，合理使用闭包可以显著提升代码的灵活性和可读性。理解和掌握闭包有助于更好地利用 Python 语言的特性，编写高效、简洁的代码。

## 题目29：说一下Python中的多线程和多进程的应用场景和优缺点。

线程是操作系统分配CPU的基本单位，进程是操作系统分配内存的基本单位。
通常我们运行的程序会包含一个或多个进程，而每个进程中又包含一个或多个线程。
多线程的优点在于多个线程可以共享进程的内存空间，所以线程间的通信非常容易实现；但是如果使用官方的CPython解释器，多线程受制于GIL（全局解释器锁），并不能利用CPU的多核特性，这是一个很大的问题。
...
## 题目001: 在Python中如何实现单例模式
Python 的全局解释器锁（Global Interpreter Lock, GIL）是 CPython 解释器实现的一部分。GIL 的主要作用是确保只有一个线程能够执行 Python 字节码，从而避免多线程并发执行时的线程安全问题。以下是对 GIL 作用的详细解释：

### GIL 的作用和目的

1. **线程安全**：
   - Python 的某些底层对象（如列表、字典）并不是线程安全的。GIL 确保在多线程环境下，每次只有一个线程执行 Python 代码，从而避免了竞争条件和其他并发问题。

2. **简化内存管理**：
   - GIL 简化了 CPython 解释器中对象引用计数的内存管理。当多个线程同时操作引用计数时，GIL 确保引用计数的操作是原子的，从而避免了内存管理的复杂性。

3. **兼容性和历史原因**：
   - GIL 的设计最初是为了简化单核处理器上的多线程编程。在多核处理器普及之前，GIL 的性能影响较小。尽管多核处理器现在很常见，GIL 仍然存在于 CPython 中，主要是由于历史原因和向后兼容性。

### GIL 的影响

1. **多线程性能瓶颈**：
   - 在多核处理器上，GIL 限制了 Python 多线程程序的并行执行性能。尽管多个线程可以同时存在，但在任意时刻，只有一个线程能够执行 Python 代码，这导致无法充分利用多核 CPU 的优势。

2. **多线程适用场景**：
   - GIL 对于 I/O 密集型任务（如网络请求、文件读写）影响较小，因为这些操作会释放 GIL，允许其他线程运行。然而，对于 CPU 密集型任务，多线程的性能提升有限。

3. **多进程解决方案**：
   - 为了绕过 GIL 的限制，可以使用多进程（multiprocessing）而不是多线程。多进程每个进程都有自己的 Python 解释器和 GIL，因此可以充分利用多核 CPU 的优势。
...
> **点评**：这道题考查到了两个知识点：
>
> 1. Python中的MRO（方法解析顺序）。
> 在没有多重继承的情况下，向对象发出一个消息，如果对象没有对应的方法，那么向上（父类）搜索的顺序是非常清晰的。
> 如果向上追溯到`object`类（所有类的父类）都没有找到对应的方法，那么将会引发`AttributeError`异常。
> 但是有多重继承尤其是出现菱形继承（钻石继承）的时候，向上追溯到底应该找到那个方法就得确定MRO。
> Python 3 中的类以及Python 2 中的新式类使用[C3算法](<https://www.jianshu.com/p/a08c61abe895>)来确定MRO，它是一种类似于广度优先搜索的方法；
> Python 2中的旧式类（经典类）使用深度优先搜索来确定MRO。
> 在搞不清楚MRO的情况下，可以使用类的`mro`方法或`__mro__`属性来获得类的MRO列表。
> 2. `super()`函数的使用。在使用`super`函数时，可以通过`super(类型, 对象)`来指定对哪个对象以哪个类为起点向上搜索父类方法。
> 所以上面`B`类代码中的`super(B, self).who()`表示以B类为起点，向上搜索`self`（D类对象）的`who`方法，所以会找到`C`类中的`who`方法，因为`D`类对象的MRO列表是`D --> B --> C --> A --> object`。
...
## 题目35：如何剖析 Python 代码的执行性能？

剖析代码性能可以使用Python标准库中的`cProfile`和`pstats`模块。
`cProfile`的`run`函数可以执行代码并收集统计信息，创建出`Stats`对象并打印简单的剖析报告。`Stats`是`pstats`模块中的类，它是一个统计对象。当然，也可以使用三方工具`line_profiler`和`memory_profiler`来剖析每一行代码耗费的时间和内存，这两个三方工具都会用非常友好的方式输出剖析结构。
...
`KeyError` 是在访问字典中不存在的键时触发的。
`TypeError` 是在操作或调用一个对象的类型不符合预期时触发的。
`ValueError` 是在一个函数接收到一个参数值，但该值的类型是正确的，但是值不适合操作时触发的。
...
> **点评**：Python函数在定义的时候，默认参数`items`的值就被计算出来了，即`[]`。
> 因为默认参数`items`引用了对象`[]`，每次调用该函数，如果对`items`引用的列表进行了操作，下次调用时，默认参数还是引用之前的那个列表而不是重新赋值为`[]`，所以列表中会有之前添加的元素。
> 如果通过传参的方式为`items`重新赋值，那么`items`将引用到新的列表对象，而不再引用默认的那个列表对象。
> 这个题在面试中经常被问到，通常**不建议使用容器类型的默认参数**，像PyLint这样的代码检查工具也会对这种代码提出质疑和警告。
...
在使用面向对象编程语言的时候，定义类是最常见的一件事情，有的时候我们会用到只有属性没有方法的类，这种类的对象通常只用于组织数据，并不能接收消息，所以我们把这种类称为数据类或者退化的类，就像C语言中的结构体那样。
...
命名元组与普通元组一样是不可变容器，一旦将数据存储在`namedtuple`的顶层属性中，数据就不能再修改了，也就意味着对象上的所有属性都遵循“一次写入，多次读取”的原则。
...
[Python面试宝典-基础篇-2020.md](https://github.com/jackfrued/Python-Interview-Bible/blob/master/Python%E9%9D%A2%E8%AF%95%E5%AE%9D%E5%85%B8-%E5%9F%BA%E7%A1%80%E7%AF%87-2020.md)  
