...
迭代器是实现了迭代器协议的对象。跟其他编程语言不通，Python中没有用于定义协议或表示约定的关键字，像`interface`、`protocol`这些单词并不在Python语言的关键字列表中。
...
在 Python 编程中，鸭子类型（Duck Typing）是一种动态类型的方式，它强调对象的行为（方法和属性）而不是对象本身的类型。简单来说，“如果它走起来像鸭子，叫起来像鸭子，那么它就是鸭子”。这意味着，只要一个对象实现了所需的方法和属性，它就可以被视为满足某种类型，而不必显式地继承某个类或实现某个接口。
示例：
```python
class Duck:
    def quack(self):
...
闭包是指在函数内部定义的函数，可以引用外部函数的变量，并且即使外部函数已经返回，内部函数仍然可以访问这些变量。闭包是一种强大的编程工具，它可以用于创建函数工厂、实现数据封装等。
