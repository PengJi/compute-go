...
static void pr_spte(unsigned long address, u64 spte, unsigned long i, int level)
{
	if (level == 1)
...
static inline void print_gpa_from_guest(unsigned long a0) {
  ...
  pxx_idx[0] = (gpa_from_guest >> 39) & mask;
  pxx_idx[1] = (gpa_from_guest >> 30) & mask;
  pxx_idx[2] = (gpa_from_guest >> 21) & mask;
  pxx_idx[3] = (gpa_from_guest >> 12) & mask;

  pr_info(" EPT PGD index: %lu", pxx_idx[0]);
  pr_info(" EPT PUD index: %lu", pxx_idx[1]);
  pr_info(" EPT PMD index: %lu", pxx_idx[2]);
  pr_info(" EPT PTE index: %lu", pxx_idx[3]);

  // pr_info("100110010 101101000 111010000 101000110 110010001000");
	pr_info("          %lu       %lu       %lu       %lu", pxx_idx[0], pxx_idx[1], pxx_idx[2], pxx_idx[3]);
