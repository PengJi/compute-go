...
static inline u64 print_huge_pte(u64 ent) {
	u64 page_hpa = ent & PT64_DIR_BASE_ADDR_MASK,    // Page HPA
		offset_mask = PT64_LVL_OFFSET_MASK(2) | (PAGE_SIZE - 1), // Page offset
	...
	pr_info(" PMD    " PTE_PATTERN " (PMD Entry that maps Huge Page)", UL_TO_PTE(ent));
	pr_info(" Mask1  " PTE_PATTERN " (get HPA of Huge Page)", UL_TO_PTE(PT64_DIR_BASE_ADDR_MASK));
	pr_info(" HFN    " PTE_PATTERN " (HFN of Huge Page)", UL_TO_PTE(page_hpa));

	pr_err("---------------------------------------------------------------------------------------------");

	pr_info(" GPA    " PTE_PATTERN " (GPA from Guest)", UL_TO_PTE(gpa_from_guest));
	pr_info(" Mask2  " PTE_PATTERN " (get offset in Huge Page)", UL_TO_PTE(offset_mask));
	pr_info(" Offset " PTE_PATTERN " (offset in Huge Page)", UL_TO_PTE(gpa_from_guest & offset_mask));
...
static inline u64 print_pte(u64 ent) {
	u64 page_hpa = ent & PT64_BASE_ADDR_MASK,    // Page HPA
	...
	pr_info(" PTE    " PTE_PATTERN " (PTE Entry that maps Page)", UL_TO_PTE(ent));
	pr_info(" Mask1  " PTE_PATTERN " (get HPA of Page)", UL_TO_PTE(PT64_BASE_ADDR_MASK));
	pr_info(" HFN    " PTE_PATTERN " (HFN of Page)", UL_TO_PTE(page_hpa));

	pr_err("---------------------------------------------------------------------------------------------");

	pr_info(" GPA    " PTE_PATTERN " (GPA from Guest)", UL_TO_PTE(gpa_from_guest));
	pr_info(" Mask2  " PTE_PATTERN " (get offset in Page)", UL_TO_PTE(offset_mask));
	pr_info(" Offset " PTE_PATTERN " (offset in Page)", UL_TO_PTE(gpa_from_guest & offset_mask));
...
static void __mmu_spte_walk1(struct kvm_vcpu *vcpu, struct kvm_mmu_page *sp,
			    inspect_spte_fn1 fn, int level)
{
	int i;
		...
		if (is_shadow_present_pte(ent[i])) {
			...
			if (i == pxx_idx[PT64_ROOT_4LEVEL - (5 - level)]) {
				...
				if (!is_last_spte(ent[i], 5 - level)) {// continue walking
					...
					__mmu_spte_walk1(vcpu, child, fn, level + 1);
				} else {
...
void mmu_spte_walk1(struct kvm_vcpu *vcpu, inspect_spte_fn1 fn)
{
	int i;
	struct kvm_mmu_page *sp;
